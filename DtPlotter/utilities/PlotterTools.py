...
class PlotterTools:
	...
	def __init__(self, args):
		...
		self.savepath = self.output_location()
	...
	def analysis_path(self):
		...
		if self.dir is None:  self.dir = self.defaultanalysispath					# Use default directory if no -d flag raised
			...
			if file.endswith('.root'):
				...
				if self.energy is not None:							# Energy specified, only adds files with that energy
					if file.split('_')[-2]==self.energy:
						Files.append( [ self.dir + file, energy_position ] )
						print "Found file: ", self.dir + file
	...
	def find_center(self, filei):	
		...
		with open(self.savepath+xtal[2]+'_log.txt', 'a') as f:
	...
	def dt_adjustment(self, filei, p, tree, cut):
		xtal 	     = self.get_xtals(filei)
		ampbias      = self.amp_coeff(xtal)
	...
	def define_cuts(self, filei):
		...
		with open(self.savepath+xtal[2]+'_log.txt','w') as f:
			f.write(xtal[2]+" "+self.energy+" energy log file\n\n")
		...
		position_cut = "(fabs(X[0]-{})<4) && (fabs(Y[0]-{})<{})".format(x_center, y_center, self.poscut)
	...
	def define_plots(self, filei):
		...
		xtal	= self.get_xtals(filei)
		ampbias = self.amp_coeff(xtal)
		Aeff    = "pow( 2 / ( (1/pow(fit_ampl[{}]/b_rms[{}], 2)) + (1/pow({}*fit_ampl[{}]/b_rms[{}],2)) ) , 0.5)".format(xtal[0],xtal[0],ampbias,xtal[1],xtal[1])		
